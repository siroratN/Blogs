...
import { editBlogSchema } from "@/lib/validate";
...
interface EditBlogInput {
    ...
    blogImagesMeta: Array<
        { type: "url"; imageUrl: string } | { type: "file"; fileIndex: number }
    >;
    extraImageFiles: File[];
}
...
function validateImageFile(file: File, fieldName: string) {
    ...
    if (file.size > maxSize) {
        throw new Error(
            `ไฟล์รูปภาพ${fieldName}มีขนาดใหญ่เกินกว่าที่กำหนด (ต้องไม่เกิน 10MB)`
        );
    }
    ...
    if (!fileExtension || !allowedExtensions.includes(fileExtension)) {
        throw new Error(
            `รูปแบบไฟล์รูปภาพ${fieldName}ไม่ถูกต้อง (รองรับเฉพาะ .png, .jpg, .jpeg เท่านั้น)`
        );
    }
}
...
export async function createBlogService(input: CreateBlogInput) {
    ...
    try {
        ...
        if (existingBlog) {
            generatedSlug = `${generatedSlug}-${Math.random()
                .toString(36)
                .substring(2, 7)}`;
        }
        ...
        if (coverImageFile && coverImageFile.size > 0) {
            ...
            const {
                data: { publicUrl },
            } = supabase.storage.from("blog-bucket").getPublicUrl(filePath);
...
export async function editBlogService(input: EditBlogInput) {
    const validatedInput = editBlogSchema.parse(input);
    ...
    try {
        ...
        for (const meta of blogImagesMeta) {
            if (meta.type === "url") {
                if (meta.imageUrl) uploadedExtraUrls.push(meta.imageUrl);
            } else if (meta.type === "file" && meta.fileIndex !== undefined) {
        ...
        if (oldPathsToDeleteAfterSuccess.length > 0) {
            await supabase.storage
                .from("blog-bucket")
                .remove(oldPathsToDeleteAfterSuccess);
        }
        ...
        if (newUploadedPathsForRollback.length > 0) {
            await supabase.storage
                .from("blog-bucket")
                .remove(newUploadedPathsForRollback);
        }
        throw error;
    }
}
