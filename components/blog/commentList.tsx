    ...
    return (
        <div className="min-h-screen bg-[#FFF5E4] font-sans">
            ...
            <div className="max-w-6xl mx-auto px-6 py-6 space-y-4 ">
                {/* Filter Tabs */}
                <div className="sticky top-24 grid grid-cols-2 md:flex gap-1 bg-white p-1 rounded-xl border border-[#FFE3E1] shadow-sm w-full md:w-fit">
                    {(["all", "pending", "approved", "rejected"] as const).map((f) => (
                        <button
                            key={f}
                            onClick={() => setFilter(f)}
                            className={`w-full md:w-auto inline-flex items-center justify-center px-3 py-2 rounded-lg text-xs font-medium transition-all ${filter === f ? "bg-[#FF9494] text-white shadow-sm" : "text-[#5C2D2D] hover:bg-[#FFF5E4]"
                                }`}
                        >
